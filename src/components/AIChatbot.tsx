...
import {
  MessageCircle, X, Send, Bot, User, AlertTriangle,
  Zap, Trash2, ChevronDown, Sparkles, Shield
} from 'lucide-react';
      ...
      <div className="flex gap-2 justify-start">
        ...
        <div className="max-w-xs">
          <div className="px-4 py-3 rounded-2xl rounded-tl-sm text-sm"
            ...
            <p>🔒 Je ne peux pas répondre à cette question. Elle concerne des informations confidentielles d'AYIHA BOOST.</p>
            <p className="mt-2 text-xs opacity-75">Pour toute demande urgente, contactez directement l'équipe sur WhatsApp : <strong>0156202023</strong></p>
          </div>
          <div className="text-xs mt-1 ml-1" style={{ color: dark ? 'var(--text-faint)' : '#aaa' }}>
  ...
  return (
    <div className={`flex gap-2 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`max-w-[75%] ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
        ...
        />
        <div className="text-xs mt-1 mx-1" style={{ color: dark ? 'var(--text-faint)' : '#bbb' }}>
          {msg.timestamp.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex gap-2">
      <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"
        style={{ background: 'linear-gradient(135deg,#00A651,#009045)' }}>
        <Bot size={13} color="white" />
      </div>
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        {
          id: `err-${Date.now()}`,
          role: 'assistant',
          content: '⚠️ Une erreur est survenue. Veuillez réessayer ou contacter notre équipe sur WhatsApp : 0156202023',
          timestamp: new Date(),
          isError: true,
        }
      ]);
      ...
      {open && (
        <div
          ...
          style={{
            ...
            height: minimized ? 'auto' : 'min(580px, calc(100vh - 140px))',
            background: panelBg,
            borderRadius: '20px',
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center"
                  style={{ background: 'rgba(0,166,81,0.2)', border: '2px solid #00A651' }}>
              ...
              <div>
                ...
                <div className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>
                  {isTyping ? (
                    <span style={{ color: '#00c960' }}>En train d'écrire...</span>
                  ) : (
            ...
            <div className="flex items-center gap-1">
              ...
              >
                <ChevronDown size={14} color="rgba(255,255,255,0.7)"
                  style={{ transform: minimized ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.3s' }} />
              </button>
              <button
          ...
          {!minimized && (
            <>
              ...
              >
                ...
                {showQuick && messages.length === 1 && !isTyping && (
                  <div className="space-y-2 pt-2">
                    <p className="text-xs font-semibold" style={{ color: dark ? 'var(--text-faint)' : '#aaa' }}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {QUICK_QUESTIONS.map(q => (
                        <button
                          ...
                          style={{
                            background: dark ? 'rgba(0,166,81,0.1)' : 'rgba(0,166,81,0.08)',
                            color: '#00A651',
                            border: '1px solid rgba(0,166,81,0.25)',
                            cursor: 'pointer',
                            fontFamily: 'Inter, sans-serif',
                          }}
                          onMouseEnter={e => (e.currentTarget.style.background = 'rgba(0,166,81,0.18)')}
                          onMouseLeave={e => (e.currentTarget.style.background = dark ? 'rgba(0,166,81,0.1)' : 'rgba(0,166,81,0.08)')}
                        >
                          {q.label}
                        </button>
                      ))}
              ...
              <div
                ...
                style={{ borderTop: `1px solid ${borderC}`, background: panelBg }}
              >
                <div className="flex items-center gap-2">
                  <input
                    ...
                    style={{
                      ...
                      background: inputBg,
                      color: dark ? 'var(--text-primary)' : '#111',
                      fontSize: '13px',
                      fontFamily: 'Inter, sans-serif',
                      outline: 'none',
                  ...
                  <button
                    ...
                    style={{
                      background: input.trim() && !isTyping
                        ? 'linear-gradient(135deg,#00A651,#009045)'
                        : dark ? 'rgba(255,255,255,0.08)' : '#f0f0f0',
                      border: 'none',
                  ...
                  >
                    <Send size={15}
                      color={input.trim() && !isTyping ? 'white' : (dark ? 'rgba(255,255,255,0.3)' : '#ccc')} />
                  </button>
                </div>
                <div className="text-center mt-2">
                  <span className="text-xs" style={{ color: dark ? 'rgba(255,255,255,0.2)' : '#ccc' }}>
                    ⚡ Propulsé par Gemini AI · AYIHA BOOST BEN/AFRICA
                  </span>
                </div>
              </div>
            </>
